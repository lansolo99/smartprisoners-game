    ...
    <div ref="puzzleContent" class="puzzleContent">
      ...
      >
        <div
          ...
          class="shapeSlot"
        >
          <div class="shapeSlot__uid">{{ index + 1 }}</div>
          <!-- Is a riddle slot -->
          <div
            v-if="slot.isMasked"
          ...
          >
            ...
            <transition name="riddleContent">
              <div v-if="!slot.riddle" class="shapeSlot__maskId">
                {{ slot.maskId }}
              </div>
            </transition>
          </div>
...
.shapeSlot {
  padding: 2px;
  display: flex;
  border: 3px dotted rgba($color-granny-smith, 0.1);
  border-radius: 15px;
  position: relative;
  &:nth-child(odd) {
